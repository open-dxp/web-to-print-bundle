...
opendxp.registerNS("opendxp.document.printpages.pdfpreview");
/**
 * @private
 */
opendxp.document.printpages.pdfpreview = Class.create({
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            ...
            this.generateButton = new Ext.Button({
                ...
                iconCls: "opendxp_material_icon_pdf opendxp_material_icon",
            ...
            this.statusUpdateBox = Ext.create('Ext.Panel', {
                ...
                items: [this.progressBar, {
                    ...
                    handler: function() {
                        Ext.Ajax.request({
                            url: Routing.generate('opendxp_bundle_web2print_document_printpage_cancelgeneration'),
                            method: 'DELETE',
                            params: {id: this.page.id},
                            success: function(response) {
                                var result = Ext.decode(response.responseText);
                                if(!result.success) {
                                    opendxp.helpers.showNotification(t('web2print_cancel_generation'), t('web2print_cancel_generation_error'), "error");
                                }
                            }.bind(this)
                        });
                    }.bind(this)
            ...
            this.downloadButton = new Ext.Button({
                ...
                handler: function () {
                    var date = new Date();
                    var url = Routing.generate('opendxp_bundle_web2print_document_printpage_pdfdownload', {id: this.page.id, download: 1, time: date.getTime()});
                    opendxp.helpers.download(url);
                }.bind(this)
            });
            ...
            this.layout = new Ext.Panel({
                ...
                items: [{
                    region: "center",
                    hideMode: "offsets",
                    bodyCls: "opendxp_overflow_scrolling opendxp_preview_body",
                    forceLayout: true,
                    autoScroll: true,
                    border: false,
                    scrollable: false,
    ...
    getProcessingOptionsGrid: function() {
        ...
        this.processingOptionsStore = new Ext.data.JsonStore({
            proxy: {
                url: Routing.generate('opendxp_bundle_web2print_document_printcontainer_getprocessingoptions'),
                type: 'ajax',
                reader: {
                    type: 'json',
    ...
    generatePdf: function() {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_bundle_web2print_document_printpage_startpdfgeneration'),
            method: 'POST',
            jsonData: params,
            success: function(response) {
    ...
    loadCurrentPreview: function () {
        var date = new Date();
        var url = Routing.generate('opendxp_bundle_web2print_document_printpage_pdfdownload', {id: this.page.id, time: date.getTime()});
    ...
    checkForActiveGenerateProcess: function() {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_bundle_web2print_document_printpage_activegenerateprocess'),
            method: 'POST',
            params: {id: this.page.id},
            success: function(response) {
                var result = Ext.decode(response.responseText);
    ...
    checkPdfDirtyState: function() {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_bundle_web2print_document_printpage_checkpdfdirty'),
            params: {id: this.page.id},
            success: function(response) {
                result = Ext.decode(response.responseText);
                if(result.pdfDirty) {
